...
RSpec.describe('v1/users', type: :request) do
  ...
  let(:invalid_attributes) do
    { name: '', last_name: ' ', age: -1, anything: 'anything' }
  end
  ...
  describe 'PATCH /update' do
    context 'with valid parameters' do
      let(:new_attributes) do
        { name: 'Pedro', username: 'Bear' }
      end

      it 'updates the requested user' do
        user = User.create!(valid_attributes)
        patch v1_user_url(user), params: { user: new_attributes }, headers: valid_headers, as: :json
        user.reload

        expect(user.name).to eq('Pedro')
        expect(user.username).to eq('Bear')
      end
    ...
    context 'with invalid parameters' do
      it 'renders a JSON response with errors for the user' do
        user = User.create!(valid_attributes)
        patch v1_user_url(user), params: { user: invalid_attributes }, headers: valid_headers, as: :json
